...
const experience = [
  {
    ...
    role: {
      en: "Project Leader – Systems Development",
      es: "Líder de Proyectos – Desarrollo de Sistemas",
    },
    period: "Sept 2024 — Actualidad",
    highlights: [
      {
        en: "Leading digital systems development projects at the largest commercial bank in the Dominican Republic",
        es: "Lidero la ejecución de proyectos tecnológicos, asegurando alineación con objetivos de negocio y cumplimiento de estándares técnicos",
      },
      {
        en: "Coordinating between engineering, compliance, legal, and operations stakeholders",
        es: "Participo en la planificación estratégica de proyectos mediante estimación de costos, tiempos y recursos",
      },
      {
        en: "Driving UX-informed design proposals within the systems development pipeline",
        es: "Coordino la integración de sistemas y aplicaciones, garantizando calidad, estabilidad y continuidad operativa",
      },
      {
  ...
  },
  {
    company: "DEXTRA",
    role: {
      en: "Junior Developer – SQL, C#, .NET",
      es: "Desarrollador Junior – SQL, C#, .NET",
    },
  ...
  },
  {
    ...
    highlights: [
      {
        en: "Managed operational workflows across multiple teams",
        es: "Lideré un equipo de instalación de electrodomésticos, asegurando la correcta ejecución de los servicios en campo",
      },
      {
        en: "Developed and implemented process improvements that reduced operational friction",
        es: "Planifiqué y optimicé rutas de instalación a nivel ciudad, mejorando la eficiencia operativa y tiempos de entrega",
      },
      {
  ...
  },
  {
    ...
    role: {
      en: "Data Analyst / Technical Support (Internship)",
      es: "Analista de Datos / Soporte Técnico (Pasantía)",
    },
...
const education = [
  ...
  },
  {
    name: {
      ...
      es: "Certificado de Diseño UX de Google",
    },
    org: "Coursera / Google",
    year: "2024",
  },
    ...
    <section className="container-wide py-20 pb-32">
      ...
      <div className="flex flex-col lg:flex-row items-start gap-12 lg:gap-16 mb-20">
        ...
        <div className="flex-shrink-0 w-full lg:w-auto">
          <div className="border border-line bg-surface p-3 rounded-3xl inline-block anim-fade-up anim-delay-2">
            <Image
              src="/profile.png"
              alt="Luis Lapaix Profile"
              width={280}
              height={280}
              className="w-full lg:w-[280px] h-auto rounded-2xl object-cover"
              priority
            />
          </div>
        </div>
          ...
          <div className="md:w-1/3">
            ...
            <div className="space-y-3">
              {t.glanceRows.map(({ label, value }) => (
                <div
                  key={label.en}
                  className="flex items-baseline justify-between gap-4 pb-3 border-b border-surface last:border-0 last:pb-0"
                >
                  <span className="text-[12px] text-fg-3">{label[lang]}</span>
                  <span className="text-[13px] font-medium text-fg-5">
                    {value[lang]}
                  </span>
                </div>
              ))}
            ...
            <div className="grid sm:grid-cols-2 gap-2.5">
              {t.strengths.map((s) => (
                <div key={s.en} className="flex items-start gap-2.5">
                  ...
                  />
                  <span className="text-[13px] text-fg-2 leading-snug">
                    {s[lang]}
                  </span>
                </div>
              ))}
                ...
                .map((exp, i, arr) => (
                  <div key={i} className="flex gap-5">
                    <div className="flex flex-col items-center gap-1 pt-1 flex-shrink-0">
                      ...
                      {i < arr.length - 1 && (
                        <div className="w-px flex-1 bg-line min-h-[40px]" />
                      )}
                    </div>
                    <div className="pb-2">
                      <div className="flex flex-wrap items-baseline gap-2 mb-0.5">
                        <h3 className="text-[14px] font-semibold text-fg tracking-[-0.01em]">
                          {exp.role[lang]}
                        </h3>
                        ...
                        {exp.highlights
                          ...
                          .map((h, j) => (
                            <li key={j} className="flex items-start gap-2">
                              <span className="text-fg-4 mt-1.5 text-[8px]">
                                ▸
                              </span>
                              <span className="text-[13px] text-fg-2 leading-snug">
                                {h[lang]}
            ...
            <div className="space-y-5">
              {t.skillGroups.map((group) => (
                <div key={group.category.en}>
                  <p className="text-[11px] font-medium text-fg-4 uppercase tracking-[0.08em] mb-2.5">
                    {group.category[lang]}
                  </p>
          ...
          <div className="rounded-xl border border-line bg-surface p-6">
            ...
            <div className="space-y-4">
              {education.map((edu) => (
                <div
                  key={edu.name.en}
                  className="flex justify-between items-start gap-4 pb-4 border-b border-surface last:border-0 last:pb-0"
                >
                  <div>
                    <p className="text-[13px] font-medium text-fg-5 leading-snug">
                      {edu.name[lang]}
                    </p>
                    <p className="text-[11px] text-fg-3 mt-0.5">{edu.org}</p>
                  </div>
                  <span className="text-[11px] font-mono text-fg-4 flex-shrink-0">
                    {edu.year}
                  </span>
                </div>
              ))}
          ...
          <div className="rounded-xl border border-line bg-surface p-6">
            ...
            <div className="space-y-1">
              {t.links.map(({ label, href }) => (
                ...
                >
                  <span className="text-[13px] text-fg-2 group-hover:text-fg transition-colors">
                    {label[lang]}
                  </span>
                  <ArrowUpRight
                    size={12}
